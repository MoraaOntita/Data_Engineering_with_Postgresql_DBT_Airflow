...
class ProdDataLoader:
    ...
    def load_data(self):
        try:
            ...
            df_track.to_sql('prod_track_table', engine, index=False, if_exists='replace')
            df_trajectory.to_sql('prod_trajectory_table', engine, index=False, if_exists='replace')
            
            logging.info("Data loaded into Prod PostgreSQL tables successfully.")
        except Exception as e:
            error_message = "Failed to load data into Prod PostgreSQL tables: {}".format(str(e))
