...
class DevDataLoader:
    ...
    def load_data(self):
        try:
            ...
            df_track.to_sql('dev_track_table', engine, index=False, if_exists='replace')
            df_trajectory.to_sql('dev_trajectory_table', engine, index=False, if_exists='replace')
            
            logging.info("Data loaded into Dev PostgreSQL tables successfully.")
        except Exception as e:
            error_message = "Failed to load data into Dev PostgreSQL tables: {}".format(str(e))
...
def main():
    try:
        data_loader = DevDataLoader("/path/to/your/data/file.csv", "your_postgres_connection_string")
